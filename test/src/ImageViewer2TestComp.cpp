...
{
  ImageViewer2TestInit(manager);
  ImageViewer2Init(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("ImageViewer2Test");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("ImageViewer2Test0");
  if (comp == nullptr)
  {
  ...
  ImageViewer2Test* testcomp = dynamic_cast<ImageViewer2Test*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
